...
export const sendWhatsAppVerificationCode = async (phone) => {
  ...
  const data = await parseJsonSafe(res);
  if (!res.ok) {
    throw new Error(data?.error?.message || data?.message || "No se pudo enviar el codigo por WhatsApp.");
  }
  return data;
};
...
export const updateStrapiUserProfile = async (userId, payload) => {
  ...
  const res = await fetch(`${STRAPI_URL}/api/users/${userId}`, buildJsonOptions("PUT", { data: payload }));
  const data = await parseJsonSafe(res);
  if (!res.ok) {
